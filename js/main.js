...
function configSigmaElements(config) {
    ...
    var a = [],
        b,x=1;
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> Komunita ' + (x++) + ' (' + sigInst.clusters[b].length + ' členů)</a></div>');
    //a.sort();
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        sigInst.draw(2, 2, 2, 2);
        $GP.info_name.html("<b>Identifikovaná komunita</b>");
        $GP.info_data.hide();
        $GP.info_p.html("Členové komunity:");
        $GP.info_link.find("ul").html(f.join(""));
        $GP.info.animate({width:'show'},350);
        $GP.search.clean();
		$GP.cluster.hide();
